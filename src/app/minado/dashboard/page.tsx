...
function generateTerminalLine(hashrate: number): string {
  const roll = Math.random();
  const now = new Date();
  const ts = `${now.getHours().toString().padStart(2,'0')}:${now.getMinutes().toString().padStart(2,'0')}:${now.getSeconds().toString().padStart(2,'0')}`;

  if (roll < 0.40) {
    return `[${ts}] ✓ Block #${randomBlockNumber()} validated — Reward: ${randomBtcReward()} BTC`;
  } else if (roll < 0.60) {
    return `[${ts}] ⟳ Scanning nonce range ${randomHex(6)}...${randomHex(6)}`;
  } else if (roll < 0.75) {
    return `[${ts}] ⟳ Pool sync: ${(POOL_MINERS + Math.floor(Math.random() * 100 - 50)).toLocaleString()} active miners`;
  } else if (roll < 0.85) {
    return `[${ts}] ✓ Share accepted — Hashrate: ${(hashrate * (0.98 + Math.random() * 0.04)).toFixed(1)} TH/s`;
  } else if (roll < 0.93) {
    return `[${ts}] ⟳ Validating merkle root ${randomHex(8)}`;
  } else {
    return `[${ts}] ⟳ Difficulty target: ${(Math.random() * 3 + 83).toFixed(2)}T — Adj in ${Math.floor(Math.random() * 10 + 2)}h`;
  }
}
...
export default function MinadoDashboardPage() {
  ...
  const userPoolSharePct = hasActivePlan
    ? ((nominalHashrate / 1e6) / POOL_TOTAL_HASHRATE_EH * 100).toFixed(6)
    : '0.000000';
  ...
  useEffect(() => {
    ...
    const seed = Array.from({ length: 8 }, () => generateTerminalLine(nominalHashrate));
    setBlockTerminal(seed);

    const id = setInterval(() => {
      const line = generateTerminalLine(liveHashrate || nominalHashrate);
      ...
      <main className="flex-1 flex flex-col min-h-0 bg-[#060608] overflow-y-auto">
        ...
        <div className="p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="rounded-2xl p-5 border border-white/5 bg-[#0d0d14] flex flex-col items-center relative overflow-hidden group hover:border-amber-500/10 transition-colors">
              ...
              <HashrateGauge
                ...
                max={Math.max(nominalHashrate * 1.15, 100)}
                unit="TH/s"
              />
              <p className="text-[10px] text-slate-500 font-mono mt-1 text-center">
                {hasActivePlan ? 'Rendimiento: 99.98% Promedio' : 'Sin potencia asignada'}
              </p>
            </div>
          ...
          <div className="rounded-2xl border border-white/5 bg-[#0d0d14] p-6 lg:p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 relative overflow-hidden">
            {/* Ambient glow */}
            {hasActivePlan && (
              <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse at 20% 50%, rgba(245,158,11,0.04) 0%, transparent 60%)' }} />
            )}
            <div className="space-y-1 relative z-10">
              ...
              <div className="text-3xl lg:text-5xl font-black tracking-tight font-mono text-white mt-2 flex items-baseline gap-2">
                {hasActivePlan ? (
                  <span className="text-amber-400 drop-shadow-[0_0_15px_rgba(245,158,11,0.3)]">
              ...
              <div className="text-slate-400 font-medium text-sm flex items-center gap-1 mt-1">
                ...
                <span className="text-[10px] font-mono text-slate-600 ml-2">(BTC/USD = ${BTC_PRICE_USD.toLocaleString()})</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Chart */}
            <div className="rounded-2xl border border-white/5 bg-[#0d0d14] p-5 flex flex-col">
              ...
              <div className="relative h-44 w-full flex items-end gap-1 p-3 rounded-xl bg-black/40 border border-white/5 overflow-hidden">
                {hasActivePlan && chartData.length > 0 ? (
                  chartData.map((val, i) => (
                    <div 
                      key={i} 
                      className="flex-1 rounded-sm cursor-pointer transition-all duration-500"
                      style={{
                        height: `${val}%`,
                        background: `rgba(245,158,11,${0.1 + (val / 100) * 0.35})`,
                        borderTop: `1px solid rgba(245,158,11,${0.3 + (val / 100) * 0.5})`,
                        boxShadow: val > 70 ? '0 0 4px rgba(245,158,11,0.3)' : 'none',
                      }}
                      title={`${((nominalHashrate * (0.95 + val / 200))).toFixed(1)} TH/s`}
                    />
                  ))
                ) : (
            ...
            <div className="rounded-2xl border border-white/5 bg-[#0d0d14] p-5 flex flex-col">
              ...
              <div
                ref={terminalRef}
                className="flex-1 h-44 overflow-y-auto font-mono text-[10px] leading-relaxed space-y-0.5 bg-black/60 rounded-xl p-3 border border-white/5"
                ...
                {hasActivePlan ? (
                  blockTerminal.map((line, i) => (
                    <div
                      ...
                      style={{
                        color: line.includes('✓') ? '#86efac' : '#94a3b8',
                        opacity: Math.max(0.3, (i / blockTerminal.length)),
                      }}
                    >
                      {line}
                    </div>
